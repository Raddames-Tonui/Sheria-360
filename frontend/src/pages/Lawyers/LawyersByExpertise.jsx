...
import "../../App.css";
import { FaPhoneAlt, FaEnvelope, FaUser } from "react-icons/fa"; // Icons
import { BiUserCircle } from "react-icons/bi"; // Default icon for missing images
...
const LawyersByExpertise = () => {
  ...
  return (
    <div className="container mx-auto p-8">
      ...
      {/* Container for each lawyer */}
      {lawyers.length > 0 ? (
        <ul className="space-y-4">
          {lawyers.map((lawyer) => (
            <li key={lawyer.id} className="border border-gray-300 p-4 rounded-md shadow-sm bg-gray-100 hover:bg-gray-200">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center p-3 border-b-2 border-gray-300">
                {/* Profile Picture */}
