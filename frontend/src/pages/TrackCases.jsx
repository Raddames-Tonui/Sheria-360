...
const TrackCases = () => {
    ...
    const stations = {
        "Milimani Law Courts": {
            ...
            "Milimani High Court": [
                ...
                { code: "C104", label: "High Court Anti Corruption and Economic Crimes" },
        ...
        "Naivasha ELC Environment and Land Court": {
            "Naivasha High Court": [
                { code: "C121", label: "High Court Div" },
            ],
    ...
    useEffect(() => {
        ...
        setJudiciaryData({ stations: transformedStations, availableCourts: [], caseCodes: [] });
    }, []);
    ...
    const handleChange = (e) => {
        ...
        if (name === "selectedStation") {
            // Reset court and caseCode when station changes
            setFormState(prevState => ({
                ...prevState,
                selectedCourt: '',
                caseCode: '',
            ...
            // Get available courts for the selected station
            const selectedStationCourts = stations[value] ? Object.keys(stations[value]) : [];
            setJudiciaryData(prevData => ({
                ...prevData,
                availableCourts: selectedStationCourts,
                caseCodes: selectedStationCourts.length > 0 ? stations[value][selectedStationCourts[0]] : [],
            }));
        }

        if (name === "selectedCourt") {
            // Update case codes based on selected court
            const selectedStation = formState.selectedStation;
            const selectedCourtCodes = selectedStation ? stations[selectedStation][value] || [] : [];
            setJudiciaryData(prevData => ({
                ...prevData,
    ...
    const handleSubmit = async (e) => {
        ...
        const { selectedStation, selectedCourt, caseCode, caseNumber, year } = formState;

        if (!selectedStation || !selectedCourt || !caseCode || !caseNumber || !year) {
            setError("Please fill in all fields.");
            return;
        }

        const serverUrl = `${server_url}`; // Update with your actual server URL
        try {
            const response = await fetch(`${serverUrl}/case/search-case?station=${encodeURIComponent(selectedStation)}&court=${encodeURIComponent(selectedCourt)}&caseCode=${encodeURIComponent(caseCode)}&caseNumber=${encodeURIComponent(caseNumber)}&year=${year}`);
            ...
            if (response.ok) {
                ...
                setError(errorData.error || "Failed to fetch case details. Please try again.");
                setCaseDetails(null); // Clear case details on error
            }
        } catch (error) {
            setError("Network error. Please try again later.");
            setCaseDetails(null); // Clear case details on network error
        }
    };
                        ...
                        <div className="flex gap-8 px-4">
                            ...
                            <div className="mb-4 w-1/3">
                                ...
                                <select
                                    ...
                                    onChange={handleChange}
                                    disabled={!judiciaryData.availableCourts?.length} // Use optional chaining
                                >
                        ...
                        <div className="grid grid-cols-3 gap-4 pt-2 bg-lime-900 px-4 rounded-md justify-between">
                            ...
                            <div className="mb-4">
                                ...
                                <select
                                    ...
                                    onChange={handleChange}
                                    disabled={!judiciaryData.caseCodes?.length} // Use optional chaining
                                >
                            ...
                            <div className="mb-4">
                                ...
                                >
                                    ...
                                    {Array.from({ length: new Date().getFullYear() - 2015 + 1 }, (_, i) => {
                                        const year = 2015 + i; // Generates years from 2015 to current year
                                        return (
